...
AirDroneCarrier = Class() {
            ...
            while self and not self.Dead and not self.BuildingDrone do
                for droneName, droneData in self.DroneData do
                    if not droneData.Active or (droneData.Active and droneData.Damaged and droneData.Docked) then
                        ...
                        ChangeState(self, self.DroneRebuildingState)
                    elseif droneData.Active and droneData.Damaged and not droneData.Docked then
                        local drone = droneData.Active
                        if drone and not drone.Dead and not drone.DockingRequired then
                            drone:DroneNavigateToDock()
                        end
                    end
                end
                WaitTicks(2)
    ...
    AssistHeartBeat = function(self)
        ...
        local IdleBeats = 0
        local IdleThreshold = math.ceil(2.0 / (self.HeartBeatInterval or 0.5))
            ...
            if self.DroneAssist and not HoldFire and SuspendAssist <= 0 then
                ...
                if NewDroneTarget and self:IsValidDroneTarget(NewDroneTarget) then
                    ...
                    and self:IsTargetInRange(LastDroneTarget) then
                        ...
                        LastDroneTarget = nil
                        IdleBeats = IdleBeats + 1
                        if IdleBeats >= IdleThreshold then
                            IdleBeats = 0
                            self:SendIdleDronesToDock()
                        end
                    end
                end
        ...
        if next(self.DroneTable or {}) then
            for name, drone in self.DroneTable do
                if drone.AwayFromCarrier == false then
                    if drone.DockingRequired then
                        drone.DockingRequired = false
                        IssueClearCommands({drone})
                    end
                    if self.DroneData[name].Docked then
                        self:UndockDrone(name)
                    end
    ...
    -- Detaches a drone from its docking bone and restores mobility.
    -- Sets JustUndocked so per-drone docking intercepts don't immediately re-attach.
    ---@param self AirDroneCarrier
    ---@param droneName string
    UndockDrone = function(self, droneName)
        local drone = self.DroneData[droneName].Active
        if drone and not drone.Dead then
            drone.JustUndocked = true
            drone:SetImmobile(false)
            drone:DetachFrom(false)
            self.DroneData[droneName].Docked = false
            drone:SetDoNotTarget(false)
            if droneName == self.BuildingDrone then
                self:CleanupDroneMaintenance(droneName)
            end
        end
    end,

    -- Sends all idle (not docked, not recalled, not already returning) drones home.
    ---@param self AirDroneCarrier
    SendIdleDronesToDock = function(self)
        if next(self.DroneTable or {}) then
            for id, drone in self.DroneTable do
                if drone and not drone.Dead
                and not self.DroneData[id].Docked
                and not drone.AwayFromCarrier
                and not drone.DockingRequired then
                    drone:DroneNavigateToDock()
                end
            end
        end
...
AirDroneUnit = Class(AirUnit) {
    ...
    OnStopBeingBuilt = function(self, builder, layer)
        local savedCarrier = self.Carrier
        ...
        if savedCarrier then
            self.Carrier = savedCarrier
            IssueGuard({self}, savedCarrier)
        end
    end,
