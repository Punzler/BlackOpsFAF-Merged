...
UAS0401 = Class(BaseTransport, ASeaUnit, AirDroneCarrier) {
    ...
    -- BAA0001 drones have no working air weapon. Dispatching them at air units causes
    -- them to hover uselessly and blocks docking until the air unit dies.
    IsValidDroneTarget = function(self, target)
        if target and not target.Dead and target.GetCurrentLayer
        and target:GetCurrentLayer() == 'Air' then
            return false
        end
        return AirDroneCarrier.IsValidDroneTarget(self, target)
    end,
